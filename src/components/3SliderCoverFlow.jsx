    ...
    { movies.length == 0 ?  <CircularProgress/> : 
     <Swiper {...swiperEffect}>
      ...
      <SwiperSlide className="coverFlowMoviesHome" key={id}>
        ...
        >
          <CardMedia component="img" alt="imagen de la película"
        ...
        <Box display={"flex"} alignItems={"center"} justifyContent={"space-around"}>
          <Button sx={{left:"-10%"}} 
            onClick={()=> toogleFavorite({ id, poster_path, title, original_title, release_date})}>
            {
              ...
              ? <Favorite sx={{fontSize:{sm:"2.1em",md:"3em"}}}/> 
              : <FavoriteBorder sx={{fontSize:{sm:"2.1em",md:"3em"}}} />
            }
          </Button> 
          <Typography sx={{fontSize:{sm:"1.3em", md:"1.5em"}}}>
